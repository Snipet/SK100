...
def  main():
  ...
  installer = "\installer\SK100 Installer.exe"
   
  if demo:
    installer = "\installer\SK100 Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SK100 manual.pdf" 
  ]
  ...
  ZIPNAME = "SK100-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SK100-v" + FULLVERSIONSTR + "-win-demo.zip"
